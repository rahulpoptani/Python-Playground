...
def solution(length, width, height):
    ...
    for i in range(len(length)):                # O(n)
        box = sorted([length[i], width[i], height[i]])
        boxes.append(box)
    
    boxes.sort(reverse=True)                    # O(n log n)
    print(boxes)

    for i in range(1, len(boxes)):              # O(n)
...
print(solution(length = [1, 3, 2], width = [1, 3, 2], height = [1, 3, 2]))
print(solution(length = [3, 1, 2], width = [3, 1, 2], height = [3, 2, 1]))